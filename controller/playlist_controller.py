...
async def add_to_playlist(user: Annotated[User, Depends(RoleChecker(allowed_roles=["user", "admin"]))], playlist_id: str,
                          items: List[Song]):
...
async def delete_playlist(user: Annotated[User, Depends(RoleChecker(allowed_roles=["user", "admin"]))], playlist_id: str):
...
async def delete_playlist_item(user: Annotated[User, Depends(RoleChecker(allowed_roles=["user", "admin"]))], playlist_id: str,
                               song_id: str):
...
async def get_playlists(category: str):
    ...
    result = [playlist.model_dump(include={'id', 'name', 'description', 'is_public', 'created_at', 'updated_at'}) for playlist
              in result]
    return result
