...
class BillboardChartTiktokTop50Spider(scrapy.Spider):
    name = 'billboard_chart_tiktoktop50'
    allowed_domains = ['billboard.com']
    start_urls = ['https://www.billboard.com/charts/tiktok-billboard-top-50/']
    ...
    def parse(self, response):
        ...
        for key, row in enumerate(response.xpath(
            ...
            last_week_position = row.xpath('li[4]/ul/li[3]/span/text()').extract_first().strip()
            peak_position = row.xpath('li[4]/ul/li[4]/span/text()').extract_first().strip()
            weeks_on_chart = row.xpath('li[4]/ul/li[5]/span/text()').extract_first().strip()
