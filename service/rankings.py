...
from model.ranking import Ranking, RankingHistoryView
...
async def get_ranking_history_by_song(chart_name: str, chart_type: str, song_title: str, song_artists: str, records: int):
    return await Ranking.find({
        'chart': chart_name,
                               'type': chart_type,
                               "ranking": {"$elemMatch": {"song_title": song_title, "song_artists": song_artists}}}).sort(-Ranking.date).project(RankingHistoryView).limit(records).to_list()
